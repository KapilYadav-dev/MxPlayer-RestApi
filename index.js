...
async function DoWork(req,res) {
    ...
    if(type==null || url ==null) res.json({status:'Error',code:'Please provide type and url params'})
    else {
        var firstUrl=await getPathName(url)
        var contentData=await getIdandDetail(process.env.FIRST_URL+'?url='+firstUrl)
        var streamdata=await getAllData(process.env.SECOND_URL+`?type=${type}&id=${contentData.data.id}`)
        if(type.includes('episode')) await SendSeriesData(contentData,streamdata,res)
...
async function SendSeriesData(contentData,streamdata,res){
    ...
    else res.json({status:'error',code:'Unable to process this url. Please check url and its type.'})
